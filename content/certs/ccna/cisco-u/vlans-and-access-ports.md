...
- A LAN is one broadcast domain: a broadcast (or unknown unicast) floods out every port except the one it came in. See [frame switching](/certs/ccna/cisco-u/frame-switching-mac-address-table/).
...
- Carrying multiple VLANs between switches also needs a trunk link on the switch-to-switch connections → [802.1Q trunking](/certs/ccna/cisco-u/8021q-trunking/).
...
- Defaults: Catalyst switches ship with VLAN 1 holding every port. VLAN 1 is also the default management VLAN, which is where the switch's own [management IP and gateway](/certs/ccna/cisco-u/switch-startup-and-verification/) live if you don't move them.
