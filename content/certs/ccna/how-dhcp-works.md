...
Covered in [common ports](/certs/ccna/common-ports-and-protocols/): `ip address dhcp` on the interface. One addition from this lesson: if the server hands over a default gateway option, the router injects a default route pointing at it. Verify with `show ip interface brief`, Method column shows DHCP.
