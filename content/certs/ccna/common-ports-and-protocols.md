...
Ports let [TCP and UDP](/certs/ccna/transport-layer-tcp-udp/) tell applications apart. IP address + port = an endpoint (socket); a TCP connection is a pair of them.
...
- Full walkthrough with URL anatomy and the GET cycle → [how HTTP works](/certs/ccna/how-http-works/).
...
- Full hierarchy and resolution chain → [how DNS works](/certs/ccna/how-dns-works/).
...
Packet-level detail, DHCP relay, and IOS server config → [how DHCP works](/certs/ccna/how-dhcp-works/).

Source 0.0.0.0 and destination 255.255.255.255 here are the reserved addresses from [reserved IPv4 addresses](/certs/ccna/reserved-ipv4-addresses/).
